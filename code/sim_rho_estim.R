...
for (i in 1:nsim){
  ...
  smpl_cov  <- cumsum(x1*x2)/(1:n)
  smpl_var1 <- cumsum(x1^2)/(1:n)
  smpl_var2 <- cumsum(x2^2)/(1:n)
  smpl_cor[, i]  <- smpl_cov / sqrt(smpl_var1 * smpl_var2)
  
  smpl_sign_cov <- cumsum(sign(x1*x2))/(1:n)
  my_cor[, i]   <- sin(pi / 2 * smpl_sign_cov)
  if (with_kendall) {
    kend_cor[, i] <- sin(
      pi / 2 * c(rep(NA_real_, 10), sapply(11:n, function(k) cor.fk(x1[1:k], x2[1:k])))
    )
  }
}


smpl_cor_q <- t(apply(smpl_cor, 1, quantile, prob = c(0.05, 0.5, 0.95)))
my_cor_q   <- t(apply(my_cor, 1, quantile, prob = c(0.05, 0.5, 0.95)))
my_asy_sd <- sqrt((1 - rho^2) * (pi^2/4 - asin(rho)^2))
my_cor_th_q <- cbind(lwr = rho + my_asy_sd * qnorm(0.05) / sqrt(1:n),
                     med = rho,
                     upr = rho + my_asy_sd * qnorm(0.95) / sqrt(1:n))
if (with_kendall) {
  kend_cor_q <- t(apply(kend_cor[-(1:10), ], 1, quantile, prob = c(0.05, 0.5, 0.95)))
  kend_cor_q <- rbind(matrix(NA, 10, 3), kend_cor_q)
}

df <- data.frame(n = 1:n, estimator = "SMPL COR (MC)", quantile = "q(0.05)", value = smpl_cor_q[, 1])
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SMPL COR (MC)", quantile = "q(0.50)", value = smpl_cor_q[, 2]))
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SMPL COR (MC)", quantile = "q(0.95)", value = smpl_cor_q[, 3]))
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SIGN COR (MC)", quantile = "q(0.05)", value = my_cor_q[, 1]))
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SIGN COR (MC)", quantile = "q(0.50)", value = my_cor_q[, 2]))
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SIGN COR (MC)", quantile = "q(0.95)", value = my_cor_q[, 3]))
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SIGN COR (ASY)", quantile = "q(0.05)", value = my_cor_th_q[, 1]))
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SIGN COR (ASY)", quantile = "q(0.50)", value = my_cor_th_q[, 2]))
df <- df %>% rbind(data.frame(n = 1:n, estimator = "SIGN COR (ASY)", quantile = "q(0.95)", value = my_cor_th_q[, 3]))
if (with_kendall) {
  df <- df %>% rbind(data.frame(n = 1:n, estimator = "KEND COR (MC)", quantile = "q(0.05)", value = kend_cor_q[, 1]))
  df <- df %>% rbind(data.frame(n = 1:n, estimator = "KEND COR (MC)", quantile = "q(0.50)", value = kend_cor_q[, 2]))
  df <- df %>% rbind(data.frame(n = 1:n, estimator = "KEND COR (MC)", quantile = "q(0.95)", value = kend_cor_q[, 3]))
}
...
ggsave("rho_estim_sv_facet.pdf", width = 6, height = 3, family = "Times")
...
sgn_cor <- apply(out$eps, 3, function(x) sin(pi/2*cumsum(sign(x[, 1]*x[, 2]))/(1:nrow(x))))
smp_cor <- apply(out$eps, 3, function(x) cumsum(Rfast::rowprods(x))/sqrt(cumsum(x[, 1]^2)*cumsum(x[, 2]^2)))
if (with_kendall) {
  for (i in 1:nsim) {
...
smpl_cor_q <- t(apply(smp_cor, 1, quantile, prob = c(0.05, 0.5, 0.95)))
my_cor_q   <- t(apply(sgn_cor, 1, quantile, prob = c(0.05, 0.5, 0.95)))
my_asy_sd <- sqrt((1 - rho^2) * (pi^2/4 - asin(rho)^2))
my_cor_th_q <- cbind(lwr = rho + my_asy_sd * qnorm(0.05) / sqrt(1:n),
                     med = rho,
                     upr = rho + my_asy_sd * qnorm(0.95) / sqrt(1:n))

# ccc_cor_q <- t(apply(ccc_cor, 1, quantile, prob = c(0.05, 0.95)))
if (with_kendall) {
  kend_cor_q <- t(apply(kend_cor[-(1:10), ], 1, quantile, prob = c(0.05, 0.5, 0.95)))
  kend_cor_q <- rbind(matrix(NA, 10, 3), kend_cor_q)
}
...
ggsave("rho_estim_garch_facet.pdf", width = 6, height = 3, family = "Times")
...
ggsave("rho_estim_mix_facet.pdf", width = 6, height = 3, family = "Times")
